...
def LbpImg():
    img_bgr = cv2.imread('mountian_road.jpg')
    ...
    return img_bgr, img_gray, img_lbp

# 讀取圖檔
img, gray, img_lbp = LbpImg()   #full screen
...
mask1[height-17:height, 230-17:230] = 255
...
# 以套用遮罩後的圖計算直方圖
hist_mask1 = cv2.calcHist([img_hsv], [0], mask1, [256], [0, 256])
hist_mask2 = cv2.calcHist([img_hsv], [0], mask2, [256], [0, 256])
hist_mask3 = cv2.calcHist([img_hsv], [0], mask3, [256], [0, 256])
...
mask_h[height-17:height, 230-17:230] = 255
mask_sel= np.zeros(gray.shape, np.uint8)

# 計算套用遮罩後的圖形
masked_gray1 = cv2.bitwise_and(gray, gray, mask = mask_h)
# 以套用遮罩後的圖計算直方圖
hist_mask1 = cv2.calcHist([img_hsv], [0], mask_h, [256], [0, 256])
...
for i in range(21):
    ...
    for j in range(31):
        ...
        masked_gray2 = cv2.bitwise_and(gray, gray, mask = mask_sel)
        hist_mask2 = cv2.calcHist([img_hsv], [0], mask_sel, [256], [0, 256])

        mask1_mask2 = cv2.compareHist(hist_mask1, hist_mask2, 3)    #Correlation
        if mask1_mask2 <= 0.858808:     #test
            cv2.rectangle(img, (x, y), (x+17, y+17), (0, 255, 0), -1)
        print(mask1_mask2, end= '')
        x += 17        
    print('\n')
    y += 17  
