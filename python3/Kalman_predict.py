...
bgsub = cv.createBackgroundSubtractorMOG2(500, 60, True) #El valor dethreshold podria variar(60)
cap = cv.VideoCapture("D:\photo\Tennis.mp4")
key = 0
...
#res = [(mu,P,mu)]
res=[]
N = 15 # para tomar un tramo inicial y ver que pasa si luego se pierdela observacion

sigmaM = 0.0001 # ruido del modelo
sigmaZ = 3*noise # deberia ser igual al ruido de media del proceso deimagen. 10 pixels pje.
Q = sigmaM**2 * np.eye(4)
R = sigmaZ**2 * np.eye(2)
...
while True:
    ...
    bgs = cv.medianBlur(bgs,1)
    bgs = cv.dilate(bgs,kernel,iterations=3)
    ...
    if(camshift):
        ...
        if(len(listCenterY)>4):
            ...
            if((listCenterY[-5] < listCenterY[-4]) and(listCenterY[-4] <
